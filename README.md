...
| `forceState` | state object | `{ iState: 'normal', focus: false }` | Force enter this state. An object with keys for one or both of `iState` (one of the mutually exclusive states as a string) and `focus` (as a boolean). If only one key is present, a shallow merge is done with the current state. Note that for an `active` `iState`, you must specify `[type]Active` and not just `active`. Only used in `componentWillReceiveProps` and not used for the initial render. |
| `initialState` | state object | `{ iState: 'normal', focus: true }` | Optional initial state to enter when the component is created. Same as `forceState` except used in the `constructor` to set `iState` and `componentDidMount` to set `focus`. |
| `refDOMNode` | function | `function(node) {...}` | Callback function is passed in a reference to the DOM node, and is called whenever the node changes. You shouldn't need to use this for anything related to React Interactive, but it's available in case you need to use it for other things. Note that if you need to focus/blur the DOM node, don't use this ref, instead pass in the `forceState` or `initialState` prop and set the focus to true/false. |
...
- Styles have the following precedence when merged:
  ...
  3. `normal` iState style if in the `normal` state
  3. The `style` prop
