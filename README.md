...
| `focus` | style&nbsp;object <br> or <br> options&nbsp;object <br> or <br> string | `{ border: '2px dotted green' }` <br> or... (same as above) | Same as above, except when the `focus` state style is merged with the style prop and one of the mutually exclusive state styles, the focus state style takes precedence over both. Also, the focus state can be applied to any element and toggles on click/tap. |
| `forceState` | state object | `{ iState: 'normal', focus: false }` | Force enter this state. An object with keys for `iState` (one of the 4 mutually exclusive states as a string) and `focus` (as a boolean). Can be useful for dealing with edge cases where the browser doesn't fire the necessary event. |
...
| `onTap` | function | `function(event) {...}` | Function called on touch tap. Event passed in is a `touchend` event (or a synthetic `click` event on a touch only device when not preceded by a `touchend` event, e.g. click fired by assistive tech). |
| `onTapTwo` | function | `function(event) {...}` | Function called on 2 finger tap. Event passed in is a `touchend` event. |
| `onTapThree` | function | `function(event) {...}` | Function called on 3 finger tap. Event passed in is a `touchend` event. |
