...
| `onClick` | function | `function(event) {...}` | Function called for mouse clicks, taps with 1 touch point/finger (called without any delay), synthetic click events, and enter keydown events (if the element has focus). Event passed in is a `click` event, or a `keydown` event. |
| `onMouseClick` | function | `function(event) {...}` | Function called for mouse clicks. Not called for click events generated synthetically, from touches, from space or enter keydown on buttons, or enter keydown on links. Event passed in is a `click` event. Note that `onMouseClick`, `onEnterKey`, and `onTap` are mutually exclusive and comprehensive for `onClick`, i.e. one and only one will be called every time `onClick` is called. |
| `onEnterKey` | function | `function(event) {...}` | Function called for enter keydown events (the element has to have focus to register keyboard events), and for click events generated synthetically, click events from space or enter keydown on buttons, and click events from enter keydown on links. Event passed in is a `keydown` event or a `click` event. Note that `onMouseClick`, `onEnterKey`, and `onTap` are mutually exclusive and comprehensive for `onClick`, i.e. one and only one will be called every time `onClick` is called. |
| `onTap` | function | `function(event) {...}` | Function called for touch taps with 1 touch point. Event passed in is always a `click` event (on touch tap, `node.click()` is called generating a `click` event). Note that `onMouseClick`, `onEnterKey`, and `onTap` are mutually exclusive and comprehensive for `onClick`, i.e. one and only one will be called every time `onClick` is called. |
...
| `stylePriority` | object | `stylePriority={ hover: true, hoverActive: true }` | By default the focus state style takes precedence over the iState style when merged. Use this prop to specify specific iStates whose style should take precedence over the focus state style. Note that for an `active` `iState`, you must specify `[type]Active` and not just `active`. |
